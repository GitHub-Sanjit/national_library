...
from book.models import BorrowBook, WishListBook
...
class BorrowBookView(View):
    def get(self, request, id):
        ...
        if request.user.account.balance < book.price:
            ...
            messages.success(
                request, "Borrowing successful. Balance deducted.")
        send_transaction_email(self.request.user, book.price,
                               "Borrowing Message", 'transactions/borrow_email.html')
...
class AddWishListView(View):
    def get(self, request, id):
        ...
        request.user.account.save()
        messages.success(
            request, "This Book is added to Your WishList Successfully.")
        return redirect('profile')
